...
function combineData (bufferDataArray, size) {
    let combinedBufferData = Buffer.alloc(size);

    if (size && bufferDataArray && Array.isArray(bufferDataArray) && bufferDataArray.length > 0){
        // console.log(bufferDataArray);
        // console.log(bufferDataArray.length);
        combinedBufferData = Buffer.concat(bufferDataArray, size);
    }
...
module.exports.combineData = combineData;